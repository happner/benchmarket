...
after = function(error, test) {
  ...
  // if (error) {
  //   console.error('benchmarket error:'.red, error);
  // }

  var duration = Date.now() - accum.timeStart;
  var memUsage = process.memoryUsage();
  // var heapTotalDelta = memUsage.heapTotal - accum.heapTotalStart;
...
var createMessage = function(test, samplesArray) {
  return {
    ...
    repo: conf.repo,
    file: test.file.substr(process.cwd().length + 1),
